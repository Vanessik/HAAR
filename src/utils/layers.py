...
class Denoiser(nn.Module):
    ...
    def loss(self, input, noise, sigma, mask, **kwargs):
        ...
        if self.scales == 1:
            if mask is not None:
                    return ((mask*(model_output - target)) ** 2).flatten(1).mean(1) * c_weight
            else:
                    return ((model_output - target) ** 2).flatten(1).mean(1) * c_weight
        
        if mask is not None:
            sq_error = dct((mask*(model_output - target))) ** 2
        else:
...
class DenoiserWithVariance(Denoiser):
    def loss(self, input, noise, sigma, mask, **kwargs):
        ...
        if mask is not None:
            losses = (((model_output - target) ** 2) * mask / logvar.exp() + logvar) / 2
        else:
...
class SimpleLossDenoiser(Denoiser):
    ...
    def loss(self, input, noise, sigma, mask, **kwargs):
        ...
        if mask is not None:
            ...
            return ((eps - noise).pow(2) * mask).flatten(1).mean(1)
...
class CrossAttention2d(ConditionedModule):
    ...
    def forward(self, input, cond):
        ...
        if cond['cross_padding'] is not None:
#             print(cond[self.cond_key_padding])
